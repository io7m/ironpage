...
public final class AttributeNamesTest
{
    ...
    @ForAll("probablyValid") final String text1)
  {
    Assertions.assertEquals(
      ...
      AttributeName.of(text0).compareTo(AttributeName.of(text1)));
  }
  ...
  {
    EqualsVerifier.forClass(AttributeName.class)
    ...
    return Stream.of("a", "a0", "a_0")
      .map(text -> {
        return DynamicTest.dynamicTest(
          "testValid" + text,
          () -> {
            Assertions.assertTrue(AttributeNames.isValidName(text));
            AttributeNames.checkValidName(text);
          });
      });
  }
    ...
    return Stream.of(
      ...
      .map(text -> {
        return DynamicTest.dynamicTest(
          "testInvalid" + text,
          () -> {
            Assertions.assertFalse(AttributeNames.isValidName(text));
