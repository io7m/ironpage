...
public final class AttributeTypeNamesTest
{
  ...
  {
    EqualsVerifier.forClass(AttributeTypeName.class)
    ...
    return Stream.of("a", "a0", "a_0")
      .map(text -> {
        return DynamicTest.dynamicTest(
          "testValid" + text,
          () -> {
            Assertions.assertTrue(AttributeTypeNames.isValidName(text));
            AttributeTypeNames.checkValidName(text);
          });
      });
  }
    ...
    return Stream.of(
      ...
      .map(text -> {
        return DynamicTest.dynamicTest(
          "testInvalid" + text,
          () -> {
            Assertions.assertFalse(AttributeTypeNames.isValidName(text));
