...
public final class AttributeNames
{
  ...
  public static final Pattern VALID_ATTRIBUTE_NAME_PATTERN =
   ...
   * @param name The name
   *
   * @return {@code true} if {@code name} is a valid attribute name
   */

  public static boolean isValidName(final String name)
  {
    Objects.requireNonNull(name, "name");
    return VALID_ATTRIBUTE_NAME_PATTERN.matcher(name).matches();
  }
    ...
    if (!isValidName(name)) {
      throw new IllegalArgumentException(
        new StringBuilder(64)
          .append("Not a valid attribute name: ")
          .append(name)
          .append(" (must match ")
          .append(VALID_ATTRIBUTE_NAME_PATTERN.pattern())
          .append(")")
          .toString());
    }
    return name;
